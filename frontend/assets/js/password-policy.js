// Browser mirror of backend/src/services/passwordPolicy.js.
// Keep behavior in sync; parity is enforced by
...
(function (global) {
  ...
  function isSimilarToIdentity(password, identity) {
    ...
    return identityTokens(identity).some((token) => {
      ...
      return token.length >= 5 && folded.includes(token);
    });
  }
  ...
  function scorePassword(password) {
    ...
    if (unique >= 8) score += 1;
    if (isCommonPassword(value) || value.length < 8) {
  ...
  global.TravelPasswordPolicy = {
    ...
    isSimilarToIdentity: isSimilarToIdentity,
