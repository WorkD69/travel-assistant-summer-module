    ...
    Promise.resolve(conn.ensureAuth ? conn.ensureAuth() : null)
      ...
      .then((res) => {
        ...
        const clientTrip = {
          ...
          participantIds: [uid],
          roles: roles,
          segments: window.TravelTripSync && typeof window.TravelTripSync.canonicalSharedTrip === "function"
            ? window.TravelTripSync.canonicalSharedTrip(bt, {}).segments
            : (Array.isArray(bt.segments) ? bt.segments : [])
        };
