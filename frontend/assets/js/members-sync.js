...
(function membersSyncModule(){
  ...
  function withAuth(fn){
    ...
    if (a.getToken && !a.getToken() && a.ensureAuth){
      a.ensureAuth(a.demo).then(fn).catch(fn);
    } else {
      fn();
    }
  }
  ...
  function mapInvite(inv){
    ...
    return {
      ...
      active: isActive,
      link: "https://travel.local/invite/" + (inv.token || inv.id)
    };
  }
  ...
  function reconcile(state){
    ...
    invites.forEach(function(iv){
      if (!siById[iv.id]){
        ops.push(a.createInvitation(tid, { email: iv.email || "", role: "participant" }).catch(function(){}));
  ...
  window.TravelMembersSync = { refresh: refresh };
})();
