(function tripSyncModule() {
  ...
  function parseSegments(value) {
    var segments;
    if (Array.isArray(value)) segments = value.slice();
    else if (!value || typeof value !== "string") segments = [];
    else {
      try {
        var parsed = JSON.parse(value);
        segments = Array.isArray(parsed) ? parsed : [];
      } catch (error) { segments = []; }
    }
    return segments.map(function(segment){
      return Object.assign({}, segment, {
        type: segment.type || segment.transportType || "segment",
        from: segment.from || segment.departurePlace || "",
        to: segment.to || segment.arrivalPlace || "",
        start: segment.start || segment.departureAt || "",
        end: segment.end || segment.arrivalAt || ""
      });
    });
  }

  function canonicalSharedTrip(serverTrip, existingTrip) {
    var previous = existingTrip || {};
    return Object.assign({}, previous, serverTrip, {
      id: serverTrip.id,
      title: serverTrip.title || "",
      route: serverTrip.route || "",
      startDate: serverTrip.startDate || "",
      endDate: serverTrip.endDate || "",
      start: serverTrip.startDate || "",
      end: serverTrip.endDate || "",
      status: serverTrip.status || "active",
      type: serverTrip.type || "group",
      segments: parseSegments(serverTrip.segments)
    });
  }

  function updateCanonicalTrip(tripId, patch) {
    var a = api();
    var s = store();
    if (!a || typeof a.updateTrip !== "function") {
      return Promise.reject(new Error("Сервис обновления поездки недоступен"));
    }
    return Promise.resolve(a.ensureAuth ? a.ensureAuth() : null)
      .then(function () { return a.updateTrip(tripId, patch); })
      .then(function (res) {
        var serverTrip = unwrap(res);
        if (!serverTrip || !serverTrip.id) throw new Error("Сервер вернул некорректную поездку");
        if (s && typeof s.setState === "function") {
          var current = typeof s.getState === "function" ? (s.getState() || {}) : {};
          var canonical = canonicalSharedTrip(serverTrip, current.trip);
          var update = { trip: canonical };
          if (Array.isArray(current.trips)) {
            var found = false;
            update.trips = current.trips.map(function (trip) {
              if (trip.id !== canonical.id) return trip;
              found = true;
              return Object.assign({}, trip, canonical);
            });
            if (!found) update.trips.unshift(canonical);
          }
          s.setState(update, { source: "server", action: "updateCanonicalTrip" });
          return canonical;
        }
        return canonicalSharedTrip(serverTrip, null);
      });
  }
  ...
  window.TravelTripSync = {
    patch: patch,
    updateCanonicalTrip: updateCanonicalTrip,
    canonicalSharedTrip: canonicalSharedTrip
  };
})();
